...
Transactions are saved to `data/transactions.csv`. Enter dates as `YYYY-MM-DD`, positive amounts as currency values, and transaction types as `income` or `expense`. The chart option requires a desktop environment that supports Tk.
...
The application separates input, processing, storage, reports, and output so team members can work on individual modules without changing the CLI.
